...
class FinancialRAGService:
  ...
  def _rag_core(self, user_query: str, top_k: int = 5, where_filter: dict | None = None) -> str:
    ...
    refined_query = self.rewrite_query(user_query)
  ...
  def extract_ticker_by_rule(self, query_text: str) -> str | None:
    """利用 Python 本instance規則快速匹配代號，不依賴 LLM"""
    for alias, ticker in self.TICKER_MAPPING.items():
